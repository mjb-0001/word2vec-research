...
file = open('all-text2-gamma-removed.txt', 'r', encoding='utf-8')
...
with tf.compat.v1.summary.FileWriter(log_dir, sess.graph):
    ...
    with open(os.path.join(log_dir, 'metadata.tsv'), 'w+') as file:
        ...
        file.write('Word'+'\t'+'Count'+'\t'+'Category'+'\n')

        # for i in range(len(new_count)):
        #     file.write(new_count[i,0]+'\t'+new_count[i,1]+   '\t' + 'nothing'+ '\n')

        # Initialize total_count_array with data from new_count
        for i in range(len(total_count_array)):
            total_count_array[i,0] = new_count[i,0]
            total_count_array[i,1] = new_count[i,1]
            total_count_array[i,2] = 'nothing'
        ...
        for i in range(len(new_count)):
            file.write(new_count[i,0]+'\t'+new_count[i,1]+   '\t' + total_count_array[i,2] + '\n')



                    # print(new_count[i,0]+'\t'+new_count[i,1]+   '\t' + 'determiner'+'\n')
                    # file.write(new_count[i,0]+'\t'+new_count[i,1]+   '\t' + 'determiner'+ '\n')
                # else:
                #     file.write(new_count[i,0]+'\t'+new_count[i,1]+   '\t' + 'nothing'+ '\n')

            # print(new_count[i,0]+'\t'+new_count[i,1]+'\n')
